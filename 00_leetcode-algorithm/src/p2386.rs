...
#[cfg(feature = "p2386-c1")]
pub fn k_sum(mut nums: Vec<i32>, mut k: i32) -> i64 {
    nums.sort_by_key(|&v| -v);

    let mut n = mi(nums.len().max(k as usize)); // find n that satisfy 2 ^ (n - 1) < k <= 2 ^ n
    ...
    let mut ans = 0;
    while k > 0 {
        ans = pq.pop().unwrap(); // SAFETY: there must are k numbers in pq
        k -= 1;
    }
